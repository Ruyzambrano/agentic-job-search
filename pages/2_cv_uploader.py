...
if new_cv:
    with st.sidebar:
        ...
        st.session_state["job_analysis"] = process_new_cv(
            st.session_state.desired_role, st.session_state.desired_location
        )
    st.title("Your CV")
    ...
    if st.session_state.get("job_analysis"):
        ...
        display_job_matches(
            st.session_state["job_analysis"].get("writer_data").jobs, sort_by
        )
